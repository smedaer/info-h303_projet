...
if ($actual){
    $statement = $db->prepare("SELECT AdRue,AdNumero,AdCodePostal,AdCity,Longitude,Latitude,Tel,Site,Admin FROM Etablissements WHERE Eta_ID = :actual");
    $statement->execute(array("actual" => $actual));
    $resEta = $statement->fetchAll(PDO::FETCH_ASSOC);
    $statement = $db->prepare("SELECT Prix,Couverts,Emporter,Livraison,Fermeture FROM Restaurants WHERE Rest_ID = :actual");
    $statement->execute(array("actual" => $actual));
    ...
    <tbody>
        ...
        while ($row = $statement->fetch(PDO::FETCH_ASSOC)){
            echo "<tr>";
                echo "<td>".$index++."</td>";
                //non clickable
                //echo "<td>".$row["Eta_ID"]."</td>";
                //doit actualiser la page
